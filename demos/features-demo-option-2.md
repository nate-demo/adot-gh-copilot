...
For this challenge, we will improve the photo detail modal behavior in [app.js](../wwwroot/app.js). The goal is to make the modal feel polished and easier to use.
...
1. Open [app.js](../wwwroot/app.js) and scan `bindPhotoDialogs`, where the native dialog is opened and closed.
...
6. Use the suggested prompt from Option 2 to implement the changes in `wwwroot/app.js`.
7. View change by selecting `Changes` on the bottom left of the chat
8. Look over the other options in that panel. Select `Terminal` and run `dotnet run` to view the changes in your browser.
...
- [ ] Implement modal UX improvements in `wwwroot/app.js`
