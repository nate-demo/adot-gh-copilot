...
Use the `.github/skills/dotnet-unit-testing/SKILL.md` skill to draft a first-test plan and scaffold without changing the repository, verify an evidence-based security review against `app.js` and `Program.cs`, and run uncommitted-change or selection review on the Demo 2/3 implementation. Capture the verified result, evidence, and next action from each quality check.
